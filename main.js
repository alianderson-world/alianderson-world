...
function section2() {
    ...
    sectionTwo.addEventListener("mouseenter", () => {
        ...
        // Aplicar estilos ao elemento .text-overlay p
let textOverlay = document.querySelector('.text-overlay p');
textOverlay.style.cssText = 'font-family: "Workbench", sans-serif; letter-spacing: 1px; line-height: 20px;';
...
function section4() {
    ...
    let videos = parentEl.querySelectorAll("video");
    let childEl = document.querySelectorAll(".first-project .first");

    // Animação do fundo ao entrar na seção
    sectionFour.addEventListener("mouseenter", () => {
        gsap.to(cursor, { background: "transparent" });
        gsap.to(sectionFour, { background: "#161515", color: "#fff" });
    });
    ...
    childEl.forEach((item) => {
        item.addEventListener("mouseenter", () => {
            ...
            // Exibir e iniciar o vídeo correto
            bgEl.style.display = "block";
            bgEl.load(); // Reinicia o vídeo
            bgEl.play();
